...
function Fun_SubmitOrder(obj) {
    ...
    if (formatid == "FMT002") {
        ...
        $.ajax({
            ...
            success: function (html) {
                $("#SelDepart").attr("disabled", "disabled")
                $("#txtvendor").attr("readonly", "readonly")
                $("#SelBrand").attr("disabled","disabled")
                $("#txtOrder_Number").attr("readonly", "readonly")
                $("#txtOrder_Number").val(po)
...
function QueryOriOrder() {
    if ($("#SelBrand").val()=="") {
        $("#labtxt").html("Please Select Brand");
        showDiv();
        return;
    }
    $("#hidBrand").val($("#SelBrand option:selected").text());
